...
void AsyncWebConfig::handleFormRequest(AsyncWebServerRequest *request, const char *filename)
{
  ...
  if (request->args() > 0)
  {
    if (request->hasArg(F("apName")))
      _apName = request->arg(F("apName"));
    for (uint8_t i = 0; i < _count; i++)
    {
      if (_description[i].type == INPUTCHECKBOX)
      {
        values[i] = "0";
        if (request->hasArg(_description[i].name))
          values[i] = "1";
      }
      ...
      {
        if (request->hasArg(_description[i].name))
          values[i] = request->arg(_description[i].name);
      }
    }
    if (request->hasArg(F("SAVE")) || request->hasArg(F("RST")))
    {
      writeConfig(filename);
      if (request->hasArg(F("RST")))
        ESP.restart();
    }
  }
  boolean exit = false;
